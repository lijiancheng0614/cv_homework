...
class VideoFeatures:
    ...
    def get_feature_vector(self):
        hists = list()
        magnitudes = list()
        for pos, (hist, magnitude) in enumerate(self.__get_features()):
            hists.append(hist)
            magnitudes.append(magnitude)
        avg_hists, avg_magnitudes, variances = self.__summarise_features(hists, magnitudes)
        feature_vector = np.concatenate((avg_hists.flatten(), avg_magnitudes.flatten(), variances.flatten()))
        return feature_vector
    def __summarise_features(self, hists, magnitudes):
        ...
        return avg_hists, avg_magnitudes, variances
